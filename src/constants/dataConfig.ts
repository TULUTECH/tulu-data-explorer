...
const DIMENSION_CONFIG: Record<
  DIMENSION_DATA_ENUM,
  { label: string; isSelectableForTable: boolean }
> = {
  [DIMENSION_DATA_ENUM.Date]: { label: "Date", isSelectableForTable: true },
  [DIMENSION_DATA_ENUM.CampaignId]: {
    label: "Campaign ID",
  ...
  [DIMENSION_DATA_ENUM.AdGroupName]: {
    label: "Ad-Group Name",
    isSelectableForTable: false,
  },
};
...
export const DIMENSION_OBJS: IDimensionObj[] = Object.values(
  DIMENSION_DATA_ENUM,
).map((value) => ({
  value,
...
export const METRIC_OBJS: IMetricObj[] = Object.values(METRIC_DATA_ENUM).map(
  (value) => ({
    value,
